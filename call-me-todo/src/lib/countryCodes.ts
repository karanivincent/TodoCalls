...
// Helper function to get default country based on user's locale or timezone
export function getDefaultCountry(): CountryCode {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (timezone?.includes('America/New_York') || locale?.startsWith('en-US')) {
      return countryCodes.find(c => c.code === 'US')!;
    }
    if (timezone?.includes('Europe/London') || locale?.startsWith('en-GB')) {
      return countryCodes.find(c => c.code === 'GB')!;
    }
    if (timezone?.includes('Africa/Nairobi') || locale?.startsWith('sw-KE')) {
      return countryCodes.find(c => c.code === 'KE')!;
    }
...
export function formatPhoneNumber(countryCode: string, phoneNumber: string): string {
  ...
  if (cleaned.startsWith(codeWithoutPlus)) {
    return `${countryCode}${cleaned.substring(codeWithoutPlus.length)}`;
  }
